...
# Exibir sugestões de filmes/séries com o mesmo código de gênero
if not filtered_data_title.empty:
    genre_code = filtered_data_title.iloc[0]['encoded_genre']
    
    # Garantir que não tentamos amostrar mais elementos do que realmente temos no DataFrame
    available_movies = merged_data[merged_data['encoded_genre'] == genre_code]
    
    if len(available_movies) >= 5:
        same_genre_movies = available_movies.sample(n=5, replace=False, random_state=42)
    else:
        same_genre_movies = available_movies

    st.write("Filmes/Séries com o Mesmo Código de Gênero:")
